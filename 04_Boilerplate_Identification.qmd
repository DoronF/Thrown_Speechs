...
## 2. A Unified Analysis of Phrase Proportionality
...
# add custom phrases that are obvious but may not be common enough in the ceremonial text
custom_phrases_df <- tibble(
  ngram = c(
    "honourable gentlemen of the senate",
    "gentlemen of the house of commons"
  )
) %>%
  # Add the n_size column so we can sort correctly
  mutate(n_size = str_count(ngram, "\\w+"))

intrusion_dictionary_df <- bind_rows(
...
## 4. Applying the Dictionary to Filter Policy Content
...
## 5. Save Intrusion Dictionary & Regex
