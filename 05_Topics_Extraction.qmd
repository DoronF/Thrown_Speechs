...
```{r pre-processing, echo = TRUE , cache.lazy=FALSE}
...
```{r generate-all-plots, echo=FALSE, warning=FALSE, message=FALSE, cache = TRUE}
...
all_topic_metrics <- purrr::map2_dfr(
  individual_plots_list,
  k_values,
  function(plot_list, k) {
...
knitr::kable(head(
  all_topic_metrics %>%
    select(k_value, topic, exclusivity, dominance_count, era)
))
```
...
model_quality_summary <- normalized_metrics %>%
  ...
  summarise(
    ...
    balanced_topics = sum(
      quality_score > 0.5 & quality_score <= 0.7,
      na.rm = TRUE
    ), # 50-70%
...
correlation <- cor(
  pca_data$exclusivity,
  pca_data$dominance_count,
  use = "complete.obs"
)
```
...
efficiency_analysis <- normalized_metrics %>%
  ...
  summarise(
    ...
    dominance_per_k = round(
      sum(dominance_count, na.rm = TRUE) / total_topics,
      2
    ),
    .groups = "drop"
  ) %>%
...
final_ranking <- tibble(k_value = k_values) %>%
  ...
  mutate(
    combined_score = mean(
      c(quality_rank, pca_rank, efficiency_rank),
      na.rm = TRUE
    ),
